...
?>
<?php
$ess = find_ess_by_id($_GET["id"]);
if (!$ess) {
    redirect_to("ess.php");
}

$id = $ess["id"];
$query = "SELECT * FROM ess WHERE id = {$id}";
$result = mysqli_query($conn, $query);
$view_post = mysqli_fetch_assoc($result);
$query_post_comment = "SELECT * FROM esscomments WHERE pid = {$id}";
...
if ((isset($_POST['submit']))&&(isset($_POST['comment']))) {    
    ...
    if($current_user!=$view_post["post_user"]) {
        ...
        $query_counter = "UPDATE ess SET comment_counter = {$comment_counter} WHERE id = {$id}";
        $result_counter = mysqli_query($conn, $query_counter);
    }
    $query_comment = "INSERT INTO esscomments (pid, comment, commentor, comment_time) VALUES ({$pid}, '{$comment}', '{$commentor}', '{$comment_time}')";
    $result_comment = mysqli_query($conn, $query_comment);
    if ($result_comment && mysqli_affected_rows($conn) == 1) {
        redirect_to("ess_comments.php?id=$id");
    }   
}
?>
<!DOCTYPE html>
                    ...
                    <section class="profile-feed">
                        ...
                        <div class="profile-stories">
                                ...
                                <div class="story-content">
                                    ...
                                    <footer>
                                        <!-- <a href="#" class="liked">
                                            ...
                                            <?php
                                            $count_query = "SELECT COUNT(*) FROM esscomments WHERE pid = {$id}";
                            ...
                            ?>
                                                ...
                                                <div class="user-comment-content" style="color: black;">
                                                <form method="post" action="ess_comments.php?id=<?php echo urlencode($_GET["id"]); ?>">
