...
?>
<?php
$oosa = find_oosa_by_id($_GET["id"]);
if (!$oosa) {
    redirect_to("oosa.php");
}

$id = $oosa["id"];
$query = "SELECT * FROM oosa WHERE id = {$id}";
$result = mysqli_query($conn, $query);
$view_post = mysqli_fetch_assoc($result);
$query_post_comment = "SELECT * FROM oosacomments WHERE pid = {$id}";
...
if ((isset($_POST['submit']))&&(isset($_POST['comment']))) {    
    ...
    if($current_user!=$view_post["post_user"]) {
        ...
        $query_counter = "UPDATE oosa SET comment_counter = {$comment_counter} WHERE id = {$id}";
        $result_counter = mysqli_query($conn, $query_counter);
    }
    $query_comment = "INSERT INTO oosacomments (pid, comment, commentor, comment_time) VALUES ({$pid}, '{$comment}', '{$commentor}', '{$comment_time}')";
    $result_comment = mysqli_query($conn, $query_comment);
    if ($result_comment && mysqli_affected_rows($conn) == 1) {
        redirect_to("oosa_comments.php?id=$id");
    }   
}
?>
<!DOCTYPE html>
                    ...
                    <section class="profile-feed">
                        ...
                        <div class="profile-stories">
                                ...
                                <div class="story-content">
                                    ...
                                    <footer>
                                        <!-- <a href="#" class="liked">
                                            ...
                                            <?php
                                            $count_query = "SELECT COUNT(*) FROM oosacomments WHERE pid = {$id}";
                            ...
                            ?>
                                                ...
                                                <div class="user-comment-content" style="color: black;">
                                                <form method="post" action="oosa_comments.php?id=<?php echo urlencode($_GET["id"]); ?>">
