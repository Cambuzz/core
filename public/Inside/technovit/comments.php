...
$id = $technovit["id"];
$school=$_GET["school"];
$query = "SELECT * FROM technovit WHERE id = {$id} , school='{$school}'";
$result = mysqli_query($conn, $query);
$view_post = mysqli_fetch_assoc($result);
$query_post_comment = "SELECT * FROM technovitcomments WHERE pid = {$id} , school='{$school}";
...
if ((isset($_POST['submit']))&&(isset($_POST['comment']))) {    
    ...
    if($current_user!=$view_post["post_user"]) {
        ...
        $query_counter = "UPDATE technovit SET comment_counter = {$comment_counter} WHERE id = {$id} , school='{$school}";
    ...
    if ($result_comment && mysqli_affected_rows($conn) == 1) {
        redirect_to("comments.php?id=$id&school=$school");
    }   
}
?>
<!DOCTYPE html>
