...
?>
<?php
$post = find_post_by_id($_GET["id"]);
if (!$post) {
    redirect_to("mun.php");
}

$id = $post["id"];
$query = "SELECT * FROM mun WHERE id = {$id}";
$result = mysqli_query($conn, $query);
$view_post = mysqli_fetch_assoc($result);
$query_post_comment = "SELECT * FROM comments WHERE pid = {$id}";
...
if ((isset($_POST['submit']))&&(isset($_POST['comment']))) {    
    ...
    if($current_user!=$view_post["post_user"]) {
        ...
        $query_counter = "UPDATE mun SET comment_counter = {$comment_counter} WHERE id = {$id}";
        $result_counter = mysqli_query($conn, $query_counter);
    }
    $query_comment = "INSERT INTO comments (pid, comment, commentor, comment_time) VALUES ({$pid}, '{$comment}', '{$commentor}', '{$comment_time}')";
    $result_comment = mysqli_query($conn, $query_comment);
    if ($result_comment && mysqli_affected_rows($conn) == 1) {
        redirect_to("mun_comment.php?id=$id");
    }   
}
?>
<!DOCTYPE html>
                    ...
                    <section class="profile-feed">
                        ...
                        <div class="profile-stories">
                                ...
                                <div class="story-content">
                                    ...
                                    <footer>
                                        <!-- <a href="#" class="liked">
                                            ...
                                            <?php
                                            $count_query = "SELECT COUNT(*) FROM comments WHERE pid = {$id}";
                            ...
                            ?>
                                                ...
                                                <div class="user-comment-content" style="color: black;">
                                                <form method="post" action="mun_comment.php?id=<?php echo urlencode($_GET["id"]); ?>">
